...
export default function App() {
  ...
  const [targetCalories, setTargetCalories] = useState<number>(() => {
    const saved = localStorage.getItem('fitapp_target_cal');
    return saved ? JSON.parse(saved) : 2500;
  });
  const [targetProtein, setTargetProtein] = useState<number>(() => {
    const saved = localStorage.getItem('fitapp_target_pro');
  ...
  useEffect(() => {
    localStorage.setItem('fitapp_target_cal', JSON.stringify(targetCalories));
    localStorage.setItem('fitapp_target_pro', JSON.stringify(targetProtein));
  ...
  // Cálculos de estadísticas y macros
  ...
  // Cálculo de 1RM Fórmula Epley: weight * (1 + reps / 30)
  const calc1RM = () => {
  ...
  // Estilos dinámicos basados en el tema `t`
  const dynamicStyles = {
    container: {
  ...
  return (
    <div style={dynamicStyles.container}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: t.text, margin: '0 0 4px 0' }}>FitApp 💪</h1>
          <p style={{ fontSize: '13px', color: t.textSecondary, margin: 0 }}>Ecosistema Completo y Pro</p>
        </div>
        <button
          onClick={() => setIsDarkMode(!isDarkMode)}
          style={{ ...dynamicStyles.secondaryButton, padding: '8px 12px', fontSize: '14px' }}
        >
      ...
      {activeTab === 'dashboard' && (
        <div>
          <div style={dynamicStyles.card}>
            ...
            {/* Barra de Progreso de Calorías y Macros */}
          ...
          {/* Gráfico Visual Simple de Evolución de Peso */}
          <div style={dynamicStyles.card}>
          ...
          <div style={dynamicStyles.card}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button style={dynamicStyles.secondaryButton} onClick={exportData}>Exportar JSON</button>
              <label style={{ ...dynamicStyles.secondaryButton, display: 'inline-block', textAlign: 'center', cursor: 'pointer' }}>
                Importar JSON
      ...
      {/* VISTA 3: TRACKER DE ENTRENAMIENTO Y CALCULADORA 1RM */}
      {activeTab === 'tracker' && (
        <div>
          ...
          {/* Calculadora de 1RM */}
          <div style={dynamicStyles.card}>
            <h2 style={dynamicStyles.cardTitle}>🔢 Calculadora de 1RM (Fuerza Máxima)</h2>
            <div style={{ display: 'flex', gap: '8px' }}>
              <input type="number" placeholder="Peso levantado (kg)" value={rmWeight} onChange={(e) => setRmWeight(e.target.value)} style={dynamicStyles.input} />
              <input type="number" placeholder="Repeticiones" value={rmReps} onChange={(e) => setRmReps(e.target.value)} style={dynamicStyles.input} />
            </div>
      ...
      {/* VISTA 4: NUTRICIÓN Y OBJETIVOS CONFIGURABLES */}
      {activeTab === 'nutrition' && (
        <div>
          <div style={dynamicStyles.card}>
