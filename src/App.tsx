...
export default function App() {
  ...
  // Estado para alertas activas de recordatorios e hidratación
  ...
  // Comprobador periódico de recordatorios de entrenamiento e hidratación
  useEffect(() => {
    const checkReminders = () => {
      ...
      if (profile?.reminderTime && profile.reminderTime === timeString) {
        setReminderAlert(`⏰ ¡Es hora de tu entrenamiento (${profile.name})! ¡A por ello! 🚀`);
      }
    };

    const interval = setInterval(checkReminders, 30000); // Comprobar cada 30s
  ...
  // Filtrado de ejercicios basado en material de casa seleccionado
  const getAutoGeneratedRoutine = (): AutoRoutineDay[] => {
    ...
    const poolExercises = EXERCISES.filter(ex => {
      if (ex.location !== locationFilter) return false;
      if (locationFilter === 'Casa') {
        // Si el ejercicio requiere un material específico, comprobar si el usuario lo tiene seleccionado
        const userTools = profile.homeEquipment || [];
        if (ex.equipment.toLowerCase().includes('bandas') && !userTools.some(t => t.toLowerCase().includes('bandas'))) return false;
        if (ex.equipment.toLowerCase().includes('silla') && !userTools.some(t => t.toLowerCase().includes('silla'))) return false;
        if (ex.equipment.toLowerCase().includes('barra') && !userTools.some(t => t.toLowerCase().includes('barra'))) return false;
      }
      return true;
    });

    const fallbackPool = EXERCISES.filter(ex => ex.location === locationFilter);
    const finalPool = poolExercises.length > 0 ? poolExercises : fallbackPool;

    return days.map((day, index) => {
      let focus = locationFilter === 'Casa' ? 'Full Body Calistenia' : 'Fuerza Gimnasio';
      let routineExs = finalPool.length > 0 ? finalPool : EXERCISES;
      
      if (index % 2 === 1 && finalPool.length > 2) {
        routineExs = [finalPool[1], finalPool[2], finalPool[0]];
      }
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', maxWidth: '480px', margin: '0 auto', padding: '20px', paddingBottom: '110px', backgroundColor: t.bg, color: t.text, minHeight: '100vh', boxSizing: 'border-box', position: 'relative' }}>
      ...
      {/* Alerta flotante de recordatorios */}
      {reminderAlert && (
        <div style={{ position: 'fixed', top: '15px', left: '20px', right: '20px', maxWidth: '440px', margin: '0 auto', backgroundColor: '#3b82f6', color: '#fff', padding: '12px 16px', borderRadius: '14px', zIndex: 200, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 8px 25px rgba(59, 130, 246, 0.4)', fontSize: '13px', fontWeight: '700' }}>
          <span>{reminderAlert}</span>
          <button onClick={() => setReminderAlert(null)} style={{ background: 'none', border: 'none', color: '#fff', fontSize: '16px', cursor: 'pointer' }}>✕</button>
        </div>
      )}
            ...
            {profile.workoutLocation === 'Casa' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <p style={{ fontSize: '11px', color: t.textSec, margin: 0 }}>Material disponible en casa (marca los que tengas):</p>
                  <span style={{ fontSize: '10px', color: t.primary, fontWeight: '700' }}>
                    {profile.homeEquipment?.length || 0} seleccionados
                  </span>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {DEFAULT_HOME_TOOLS.map(tool => {
                    ...
                    return (
                      <button
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          handleToggleHomeEquipment(tool);
                        }}
                        style={{
                          padding: '6px 12px', borderRadius: '8px', fontSize: '11px', cursor: 'pointer', fontWeight: '600',
                          backgroundColor: hasTool ? 'rgba(59, 130, 246, 0.2)' : t.bg,
                          color: hasTool ? t.primary : t.textSec,
                          border: `1px solid ${hasTool ? t.primary : t.border}`,
      ...
      {activeTab === 'nutricion' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ backgroundColor: t.card, borderRadius: '16px', padding: '16px', border: `1px solid ${t.border}` }}>
            ...
            <p style={{ fontSize: '11px', color: t.textSec, margin: '10px 0 0 0' }}>Recordatorio configurado: Cada {profile.waterIntervalHours || 2} horas.</p>
          </div>
          ...
          <div style={{ backgroundColor: t.card, borderRadius: '16px', padding: '16px', border: `1px solid ${t.border}` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '12px' }}>
              ...
              <div style={{ borderTop: `1px solid ${t.border}`, paddingTop: '10px', marginTop: '4px' }}>
                <strong style={{ fontSize: '12px', color: t.primary, display: 'block', marginBottom: '8px' }}>⏰ Programar Recordatorios y Alertas</strong>
